...
import { TrendingUp, Calendar, Wallet, Calculator, Percent, Repeat, Layers, Coins, CheckCircle2, ArrowRightLeft, Zap, ToggleLeft, ToggleRight } from 'lucide-react';

// --- КОМПОНЕНТ ВВОДА (ВЫНЕСЕН ДЛЯ СТАБИЛЬНОСТИ ФОКУСА) ---

const formatNum = (val) => new Intl.NumberFormat('ru-RU').format(val);

const InputBlock = ({ label, sublabel, icon: Icon, value, onChange, min, max, step, suffix, activeColor }) => (
    ...
    <div className="flex justify-between items-start">
      ...
      <div className="bg-white px-3 py-1.5 rounded-2xl border border-slate-200 shadow-inner flex items-center">
        <input 
          ...
          className={`w-24 text-right font-black text-sm focus:outline-none bg-transparent ${activeColor}`}
        />
...
const App = () => {
  ...
  // Универсальная функция расчета итогов
  const runSimulation = useCallback((rate, freq, isReinvest, isLotSize) => {
    let cap = principal;
    let cash = 0;
    ...
    for (let m = 1; m <= months; m++) {
      if (m % interval === 0) {
        ...
        if (isReinvest) {
          ...
          cash += coupon; // Купоны просто копятся
        }
      }
    }
    return cap + cash;
  }, [principal, bondPrice, years, taxRate]);

  // Расчеты для графиков и таблиц
  const { results, breakEvenRate, effSingle, effMonthly } = useMemo(() => {
    ...
    for(let i=0; i<15; i++) {
      bRate = (low + high) / 2;
      if (runSimulation(bRate, 12, reinvest, useLotSize) < target) low = bRate;
      else high = bRate;
    }

    return { results: data, breakEvenRate: bRate.toFixed(2), effSingle: getEff(finalS), effMonthly: getEff(finalM) };
  }, [principal, yieldSingle, yieldMonthly, bondPrice, years, taxRate, reinvest, useLotSize, runSimulation]);
    ...
    <div className="min-h-screen bg-[#f1f5f9] p-4 md:p-8 font-sans text-slate-900 leading-tight">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-emerald-600 p-6 rounded-[2.5rem] text-white shadow-xl shadow-emerald-200 relative overflow-hidden">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col justify-center">
            ...
            <div className="text-sm font-bold text-slate-600 leading-snug">
              При ставке {breakEvenRate}% Лестница догонит Стратегию 1
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            <div className="bg-white p-6 rounded-[3rem] shadow-xl shadow-slate-200/60 border border-white space-y-4 h-fit">
              <h2 className="text-xl font-black text-slate-700 mb-4 px-2">Параметры</h2>
              
              <InputBlock label="Стартовый капитал" value={principal} onChange={setPrincipal} min={10000} max={10000000} step={100000} suffix="₽" icon={Wallet} activeColor="text-blue-600" />
              ...
              <div className="space-y-3 pt-2">
                <div 
                  ...
                  className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${useLotSize ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-100'}`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-xl ${useLotSize ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
                      <CheckCircle2 size={18}/>
                    </div>
                    <span className={`text-xs font-black uppercase tracking-tight ${useLotSize ? 'text-blue-700' : 'text-slate-500'}`}>Учет цены лота</span>
                  </div>
                  {useLotSize ? <ToggleRight className="text-blue-500" /> : <ToggleLeft className="text-slate-300" />}
                </div>

                <div 
                  onClick={() => setReinvest(!reinvest)}
                  className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${reinvest ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-100'}`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-xl ${reinvest ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
                      <Repeat size={18}/>
                    </div>
                    <span className={`text-xs font-black uppercase tracking-tight ${reinvest ? 'text-indigo-700' : 'text-slate-500'}`}>Реинвестирование</span>
                  </div>
                  {reinvest ? <ToggleRight className="text-indigo-500" /> : <ToggleLeft className="text-slate-300" />}
                </div>
              </div>
              ...
              <div className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={results}>
                    ...
                    <Line type="monotone" dataKey="single" stroke="#6366f1" strokeWidth={5} dot={false} />
                    <Line type="monotone" dataKey="monthly" stroke="#10b981" strokeWidth={5} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
